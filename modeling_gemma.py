...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        bsz,q_len,_=hidden_sates.size() #[batch_Size,seq_len,Hidden_siae]
...
class GemmaModel(nn.Module):
    def __init__(self,config:GemmaConfig):
        ...
        self.layers =nn.ModuleList(
            [GemmaDecoderLayr(config,layer_idx)for layer_idx in range(config.num_hidden_layers)]
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def __init__(self, config:PaliGemmaConfig):
        ...
        self.multi_mode_projector = PaliGemmaMultiModelProjector(config)
        self.vocab_size = config.vocab_size
